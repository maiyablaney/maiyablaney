window.addEventListener("load", function() {
  // -----------------------------
  // SETUP: Loading Screen with Accurate Progress
  // -----------------------------
  const loadingScreen = document.querySelector('.loading-screen');
  const loadingCount = document.querySelector('.loading-count');
  let loadedResources = 0;
  const totalFrames = 40; // PNG sequence frames 0001.png to 0040.png
  const totalResources = totalFrames; // Tracking PNG sequence loading

  // Preload PNG sequence images & update progress as each loads
  const frames = [];
  ...
  function updateLoadingProgress() {
    const progressPercentage = Math.floor((loadedResources / totalResources) * 100);
    loadingCount.textContent = progressPercentage + '%';
  ...
  // In case some images are cached, update progress immediately.
  updateLoadingProgress();

  // -----------------------------
  // PNG Sequence Canvas Setup for Album Animation
  // -----------------------------
  ...
  // -----------------------------
  // Smooth Scrubbing for PNG Sequence Animation
  // -----------------------------
  let albumProgress = 0;
  ...
  // -----------------------------
  // Videos Ticker Animation (JS-Driven)
  // -----------------------------
  ...
  // -----------------------------
  // Ticker Cloning Functions (For non-special tickers)
  // -----------------------------
  ...
  // -----------------------------
  // Fumbled, Honey & Recognize Ticker Initializations
  // -----------------------------
  ...
  function initHoneyTicker() {
    ...
    const templateHTML = `<p class="ticker-text">
        <a href="https://www.google.com" target="_blank" class="ticker-link">Honey I</a>
      </p>`;
    content.innerHTML = templateHTML;
    while (content.offsetHeight < container.offsetHeight * 2) {
      content.innerHTML += templateHTML;
  ...
  // -----------------------------
  // Intro Section Effects (After Loading Screen Disappears)
  // -----------------------------
